...
public function filter_detail($id){
    ...
    if (!empty($start_date) && !empty($end_date)) {
        ...
        $data['simpanan_pokok'] = $this->SimpananPokok_model->filterByDateRangeDetail($id, $start_date, $end_date);
        $data['total_simpanan_pokok'] = $this->SimpananPokok_model->calculateTotalByDateRangeDetail($id, $start_date, $end_date);
    } else {
        // Jika tidak, gunakan tahun dan bulan
        $data['simpanan_pokok'] = $this->SimpananPokok_model->filterByYearMonthDetail($id, $year, $month);
        $data['total_simpanan_pokok'] = $this->SimpananPokok_model->calculateTotalByYearMonthDetail($id, $year, $month);
    }
...
public function filter(){
    ...
    if (!empty($start_date) && !empty($end_date)) {
        // Jika rentang tanggal dipilih, gunakan rentang tanggal
        $filtered_anggota_ids = $this->SimpananPokok_model->getAnggotaIdsByDateRange($start_date, $end_date);
        $data['anggota'] = $this->Anggota_model->getByIds($filtered_anggota_ids);
        $data['total_anggota'] = $this->SimpananPokok_model->total_simpanan_pokok_per_anggota_by_date_range($start_date, $end_date);
        $data['total_anggota_'] = $this->SimpananPokok_model->calculateTotalByDateRange($start_date, $end_date);
    } elseif (!empty($year) && !empty($month)) {
        // Jika tahun dan bulan dipilih, gunakan fungsi getAnggotaIdsByYearMonth
        $filtered_anggota_ids = $this->SimpananPokok_model->getAnggotaIdsByYearMonth($year, $month);
        $data['anggota'] = $this->Anggota_model->getByIds($filtered_anggota_ids);
        $data['total_anggota'] = $this->SimpananPokok_model->total_simpanan_pokok_per_anggota_by_year_month($year, $month);
        $data['total_anggota_'] = $this->SimpananPokok_model->calculateTotalByYearMonth($year, $month);
    } elseif (!empty($year)) {
        // Jika hanya tahun yang dipilih, gunakan fungsi getAnggotaIdsByYear
        $filtered_anggota_ids = $this->SimpananPokok_model->getAnggotaIdsByYear($year);
        $data['anggota'] = $this->Anggota_model->getByIds($filtered_anggota_ids);
        $data['total_anggota'] = $this->SimpananPokok_model->total_simpanan_pokok_per_anggota_by_year($year);
        $data['total_anggota_'] = $this->SimpananPokok_model->calculateTotalByYear($year);
    } else {
...
$excel->getActiveSheet()->getStyle('A2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); // Pusatkan teks secara horizontal
        ...
        $anggota = $this->Anggota_model->getAnggotaByFilterPokok($year, $month, $start_date, $end_date);
        $total_simpanan_pokok_per_anggota = $this->SimpananPokok_model->total_simpanan_pokok_all_detail($year, $month, $start_date, $end_date);
...
$anggota_ids = array();
 ...
 // Menghitung jumlah simpanan pokok per anggota
 $total_simpanan_pokok_anggota = isset($total_simpanan_pokok_per_anggota[$data->id_anggota]) ? $total_simpanan_pokok_per_anggota[$data->id_anggota] : '';
...
;
     ...
     $total_simpanan_pokok_anggota_rp = '';
if (is_numeric($total_simpanan_pokok_anggota)) {
    // Format jumlah simpanan pokok per anggota menjadi format Rupiah
    $total_simpanan_pokok_anggota_rp = 'Rp ' . number_format(floatval($total_simpanan_pokok_anggota), 0, ',', '.');
}

     if (!empty($start_date) && !empty($end_date)) {
        $filtered_data = $this->SimpananPokok_model->getAnggotaIdsByDateRange($start_date, $end_date);
    } elseif (!empty($year) && !empty($month)) {
        $filtered_data = $this->SimpananPokok_model->getAnggotaIdsByYearMonth($year, $month);
    } elseif (!empty($year) && empty($month)) {
        // Menangani kasus saat hanya tahun yang dipilih tanpa bulan
        $filtered_data = $this->SimpananPokok_model->getAnggotaIdsByYear($year); // Menambahkan parameter $month dengan nilai null
    } else {
        $filtered_data = $this->SimpananPokok_model->getALL();
    }
...
$total_simpanan_pokok = $this->SimpananPokok_model->total_simpanan_pokok_all_export($year, $month, $start_date, $end_date); // Sesuaikan dengan metode Anda untuk menghitung total simpanan pokok berdasarkan filter
...
// Ambil data simpanan pokok berdasarkan ID anggota
 ...
 if (!empty($start_date) && !empty($end_date)) {
    $filtered_data = $this->SimpananPokok_model->filterByDateRangeDetail($id_anggota, $start_date, $end_date);
} elseif (!empty($year) && !empty($month)) {
    $filtered_data = $this->SimpananPokok_model->filterByYearMonthDetail($id_anggota, $year, $month);
} elseif (!empty($year) && empty($month)) {
    // Menangani kasus saat hanya tahun yang dipilih tanpa bulan
    $filtered_data = $this->SimpananPokok_model->filterByYearMonthDetail($id_anggota, $year, null); // Menambahkan parameter $month dengan nilai null
} else {
    $filtered_data = $this->SimpananPokok_model->detail_simpanan_pokok_export_detail($id_anggota);
}
...
foreach ($filtered_data as $simpanan_data) {
    ...
    $html .= '<p style="text-align:center">' . $filter_text . '</p>';
    $html .= '<table border="1">';
    $html .= '<tr>';
    ...
    $html .= '<th style="text-align:center; background-color: #e0e0e0; width: 150px;">Simpanan Pokok</th>'; // Tambahkan kolom untuk simpanan pokok
    $html .= '</tr>';
    $no = 1;

    // Fetch anggota data based on parameters
    if (!empty($year) && !empty($month)) {
        $anggota = $this->SimpananPokok_model->getAnggotaIdsByYearMonth($year, $month);
    } elseif (!empty($year) && empty($month)) {
        $anggota = $this->SimpananPokok_model->getAnggotaIdsByYear($year);
    } elseif (!empty($start_date) && !empty($end_date)) {
        $anggota = $this->SimpananPokok_model->getAnggotaIdsByDateRange($start_date, $end_date);
    } else {
        $anggota = $this->SimpananPokok_model->getAll();
    }
...
foreach ($anggota as $value) {
    ...
    if (!in_array($value->id_anggota, $displayed_ids)) {
        ...
        if (isset($total_simpanan_pokok_per_anggota[$value->id_anggota])) {
    ...
    $total_simpanan_pokok_all = $this->SimpananPokok_model->total_simpanan_pokok_all_export($year, $month, $start_date, $end_date); // Sesuaikan dengan metode Anda untuk menghitung total simpanan pokok berdasarkan filter
    $total_simpanan_pokok_all_rp = 'Rp ' . number_format($total_simpanan_pokok_all, 0, ',', '.');
    $html .= '<tr>';
    $html .= '<td colspan="5" style="text-align:center; background-color: #d3d3d3; font-weight:bold">Total Simpanan Pokok</td>'; // Gabung kolom 0-4
...
if (!empty($start_date) && !empty($end_date)) {
    ...
    $filtered_data = $this->SimpananPokok_model->detail_simpanan_pokok_export_detail($id_anggota);
            ...
            foreach ($filtered_data as $pokok) {
                // Set nomor urut
                $html .= '<tr>';
                $html .= '<td style="text-align:center">' . $no++ . '</td>';
            ...
            $total_jumlah = array_sum(array_column($filtered_data, 'jumlah'));
            $html .= '<tr>';
            $html .= '<td colspan="2" style="text-align:center; background-color: #d3d3d3; font-weight:bold">Total Simpanan Pokok</td>';
            ...
            // Close and output PDF document
            $pdf->Output('datail_simpanan_pokok.pdf', 'I');
        }
    }
}
