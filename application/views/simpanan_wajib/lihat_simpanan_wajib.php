...
<?php endif; ?>
      ...
      <section class="content-header">
        <h1>
          Kelola
          <small>Data Simpanan Wajib</small>
        </h1>
        <ol class="breadcrumb">
        ...
        <div class="row">
          <div class="col-xs-12">
            <div class="box">
             ...
             <a href="<?php echo base_url("simpanan_wajib/export"); ?>?year=<?php echo $this->session->userdata('filter_year'); ?>&month=<?php echo $this->session->userdata('filter_month'); ?>&start_date=<?php echo $this->session->userdata('filter_start_date'); ?>&end_date=<?php echo $this->session->userdata('filter_end_date'); ?>" class="btn btn-carot"><i class="fa fa-fw fa-file-pdf-o"></i>Export Excel</a>
              <a href="<?php echo base_url("simpanan_wajib/export_pdf"); ?>?year=<?php echo $this->session->userdata('filter_year'); ?>&month=<?php echo $this->session->userdata('filter_month'); ?>&start_date=<?php echo $this->session->userdata('filter_start_date'); ?>&end_date=<?php echo $this->session->userdata('filter_end_date'); ?>" class="btn btn-ijo"><i class="fa fa-fw fa-file-pdf-o"></i>Export PDF</a>
              <!-- /.box-header --></div>
              <div class="col-md-6">
        <form method="post" action="<?php echo base_url("simpanan_wajib/filter"); ?>">
                  ...
                  <tbody>
                    ...
                    <?php foreach ($anggota as $value): ?>
                      <tr>
                        ...
                        <!-- Tambahkan kolom untuk menampilkan total simpanan wajib -->
            <td>
    <?php foreach ($total_anggota as $item): ?>
        <?php if ($item->id_anggota == $value->id_anggota): ?>
            <?php echo "Rp. " . number_format($item->total_simpanan_wajib, 0, ',', '.'); ?>
        <?php endif; ?>
    <?php endforeach; ?>
</td>
                        <td>
                          <a class="btn btn-primary" href="<?php echo site_url('simpanan_wajib/add/'.$value->id_anggota) ?>"><i class="fa fa-fw fa-plus"></i>Simpanan Wajib</a>
                          <a class="btn btn-success" href="<?php echo site_url('simpanan_wajib/detail/'.$value->id_anggota) ?>"></i>Detail Simpanan Wajib</a>
                        </td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
                ...
                <!-- <h2 style="text-align: center; color: #336699; font-family: Arial, sans-serif; padding: 10px; background-color: #f0f0f0; border-radius: 5px;">Total Simpanan Wajib Seluruh Anggota: <?php echo "Rp " . number_format($total, 0, ',', '.'); ?></h2> -->
                <?php if (!empty($anggota)) : ?>
    <!-- Tampilkan total simpanan wajib hanya sekali -->
    <?php if (!$this->input->post('start_date') && !$this->input->post('end_date') && !$this->input->post('month') && !$this->input->post('year')) : ?>
        <h1 class="label label-success" style="font-size: 18px;"> Total Simpanan Wajib : <?php echo "Rp. " . (number_format($total, 0, ',', '.')) ?></h1>
    <?php else: ?>
        <h1 class="label label-success" style="font-size: 18px;"> Total Simpanan Wajib Hasil Filter : <?php echo "Rp. " . (number_format($total_anggota_, 0, ',', '.')) ?></h1>
    <?php endif; ?>
<?php endif; ?>
              </div>
              <!-- /.box-body -->
