...
<?php endif; ?>
      ...
      <section class="content-header">
        ...
        <ol class="breadcrumb">
          ...
          <li><a href="<?php echo base_url('simpanan_wajib') ?>">Lihat Data Anggota</a></li>
        </ol>
      </section>
    ...
    <div class="member-info">
        <h4>Nama: <span><?php echo $simpanan_wajib[0]->nama; ?></span></h4>
        <h4>NIA: <span><?php echo $simpanan_wajib[0]->nia; ?></span></h4>
    </div>
<?php endif; ?>
<div class="box-header"  style="margin-bottom: 10px;">
<a href="<?php echo base_url("simpanan_wajib/export_detail/$id_anggota"); ?>?year=<?php echo $this->session->userdata('filter_year'); ?>&month=<?php echo $this->session->userdata('filter_month'); ?>&start_date=<?php echo $this->session->userdata('filter_start_date'); ?>&end_date=<?php echo $this->session->userdata('filter_end_date'); ?>" class="btn btn-carot"><i class="fa fa-fw fa-file-excel-o"></i>Export Excel</a>
                 <a href="<?php echo base_url("simpanan_wajib/export_detail_pdf/$id_anggota"); ?>?year=<?php echo $this->session->userdata('filter_year'); ?>&month=<?php echo $this->session->userdata('filter_month'); ?>&start_date=<?php echo $this->session->userdata('filter_start_date'); ?>&end_date=<?php echo $this->session->userdata('filter_end_date'); ?>" class="btn btn-ijo"><i class="fa fa-fw fa-file-pdf-o"></i>Export PDF</a>

              </div>

              <div class="row">
    <div class="col-md-6">
        <form method="post" action="<?php echo base_url("simpanan_wajib/filter_detail/$id_anggota"); ?>">
        ...
        <form method="post" action="<?php echo base_url("simpanan_wajib/filter_detail/$id_anggota"); ?>">
            ...
            <button type="submit" class="btn btn-primary">Filter</button>
                 ...
                 <table id="customTable" class="table table-bordered table-hover">
                  ...
                  <tbody>
                    <?php $no = 1;?>
                    <?php foreach ($simpanan_wajib as $nilai): ?>
                      <tr>
                        <td><?php cetak($no++) ?></td>
                        <!--//menambahkan 2 koma belakang <td><?php echo "Rp. " . (number_format($nilai->jumlah,2,',','.')) ?></td> -->
                        <td><?php cetak($nilai->tanggal_dibayar ) ?></td>
                        <td><?php echo "Rp. " . (number_format($nilai->jumlah, 0, ',', '.')) ?></td>
                        <td>
                          <a class="btn btn-ref" href="<?php echo site_url('simpanan_wajib/edit/'.$nilai->id_simpanan_wajib) ?>"><i class="fa fa-fw fa-edit"></i>Edit</a>
                         <a href="#!" onclick="deleteConfirm('<?php echo site_url('simpanan_wajib/delete/'.$nilai->id_simpanan_wajib) ?>')" class="btn btn-mandarin"><i class="fa fa-fw fa-trash"></i>Hapus</a>
                        </td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
    ...
    <?php foreach ($tot as $nilai): ?>
        <h1 class="label label-success" style="font-size: 18px;"> Total Simpanan Wajib : <?php echo "Rp. " . (number_format($nilai->jumlah,0,',','.')) ?></h1>
    <?php endforeach; ?>
<?php endif; ?>

                  <?php if (!empty($total_simpanan_wajib)) : ?>
    <h1 class="label label-success" style="font-size: 18px;"> Total Simpanan Wajib : <?php echo "Rp. " . (number_format($total_simpanan_wajib, 0, ',', '.')) ?></h1>
<?php endif; ?>
