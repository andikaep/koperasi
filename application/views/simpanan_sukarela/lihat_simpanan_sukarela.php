...
<?php endif; ?>
        ...
        <div class="row">
          <div class="col-xs-12">
            <div class="box">
              ...
              <a href="<?php echo base_url("simpanan_sukarela/export"); ?>?year=<?php echo $this->session->userdata('filter_year'); ?>&month=<?php echo $this->session->userdata('filter_month'); ?>&start_date=<?php echo $this->session->userdata('filter_start_date'); ?>&end_date=<?php echo $this->session->userdata('filter_end_date'); ?>" class="btn btn-carot"><i class="fa fa-fw fa-file-pdf-o"></i>Export Excel</a>
              <a href="<?php echo base_url("simpanan_sukarela/export_pdf"); ?>?year=<?php echo $this->session->userdata('filter_year'); ?>&month=<?php echo $this->session->userdata('filter_month'); ?>&start_date=<?php echo $this->session->userdata('filter_start_date'); ?>&end_date=<?php echo $this->session->userdata('filter_end_date'); ?>" class="btn btn-ijo"><i class="fa fa-fw fa-file-pdf-o"></i>Export PDF</a>
                </div>
              <!-- /.box-header -->
              <div class="col-md-6">
        <form method="post" action="<?php echo base_url("simpanan_sukarela/filter"); ?>">
 ...
 <br>
    ...
    <?php if (!$this->input->post('start_date') && !$this->input->post('end_date') && !$this->input->post('month') && !$this->input->post('year')) : ?>
        <h1 class="label label-success" style="font-size: 18px;"> Total Simpanan Sukarela : <?php echo "Rp. " . (number_format($total, 0, ',', '.')) ?></h1>
    <?php else: ?>
        <h1 class="label label-success" style="font-size: 18px;"> Total Simpanan Sukarela Hasil Filter : <?php echo "Rp. " . (number_format($total_anggota_, 0, ',', '.')) ?></h1>
    <?php endif; ?>
<?php endif; ?>
              </div>
              <!-- /.box-body -->
