      ...
      <?php if ($this->session->flashdata('success')): ?>
        <div class="box-body">
          <div class="alert alert-info alert-dismissible">
            ...
            <a href="<?php echo base_url('pinjaman/') ?>">Saya Mengerti</a>
          </div>
        </div>
      <?php endif; ?>
      <!-- Alert -->
...
<?php endif; ?>
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="<?php echo base_url('pinjaman') ?>"><i class="fa fa-fw fa-child"></i>Lihat Data Pinjaman</a></li>
        <li><a href="#">Edit Pinjaman</a></li>
      </ol>
    </section>
      ...
      <div class="row">
        ...
        <div class="col-md-8">
          ...
          <div class="box box-primary">
            ...
            <h3 class="box-title">Tambah Pinjaman: <strong><?php echo $anggota->nama; ?></strong></h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" action="<?php echo base_url('pinjaman/edit/'.$pinjaman->id_pinjaman) ?>" method="post">
              <input type="hidden" name="id_anggota" value="<?php echo $pinjaman->id_anggota?>" />

              <div class="box-body">
                <div class="form-group">
                  <label>No Pinjaman</label>
                  <input name="no_pinjaman" class="form-control <?php echo form_error('no_pinjaman') ? 'is-invalid':'' ?>" placeholder="Masukan No Pinjaman" value="<?php echo $pinjaman->no_pinjaman?>" type="text" readonly/>
                ...
                <div class="form-group">
                  ...
                  <input name="jumlah_pinjaman" id="jumlah_pinjaman" class="form-control <?php echo form_error('jumlah_pinjaman') ? 'is-invalid':'' ?>" placeholder="Masukan Jumlah Peminjaman Tanpa (.)" type="text" value="<?php echo $pinjaman->jumlah_pinjaman?>"/>
                  <div class="invalid-feedback">
    ...
    <p id="terbilang" class="form-control-static"></p>
                <div class="form-group">
                  ...
                  <input name="lama" class="form-control <?php echo form_error('lama') ? 'is-invalid':'' ?>" placeholder="Masukan Lama Peminjaman" type="text" value="<?php echo $pinjaman->lama?>"/>
                  <div class="invalid-feedback">
                ...
                <div class="form-group">
                  <label>Bunga (%)</label>
                  <input name="bunga" class="form-control <?php echo form_error('bunga') ? 'is-invalid':'' ?>" placeholder="Masukan Jumlah Bunga" type="text" value="<?php echo $pinjaman->bunga?>"/>
                  <div class="invalid-feedback">
...
<div class="modal fade" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="confirmModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <h4>Anda akan mengubah Pinjaman <strong><?php echo $anggota->nama; ?></strong></h4>
        <h4 class="text-muted">Dari <strong id="modalJumlah"><?php echo $pinjaman->jumlah_pinjaman;  ?></strong></h4>
        <h4 class="font-weight-bold mb-4">Menjadi <strong><span id="modalNewJumlah"></span></strong></h4>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        var inputJumlah = document.querySelector('input[name="jumlah_pinjaman"]');
...
function formatRupiah(angka) {
    ...
    document.querySelector('form').addEventListener('submit', function() {
        ...
        jumlah_pinjaman.value = jumlah_pinjaman.value.replace('Rp ', '').replace(/\./g, '').replace(/,/g, '.');
    });
    // Fungsi untuk mengonversi angka menjadi terbilang

    // Dapatkan elemen input
    var bungaInput = document.getElementById('bunga');

    // Tambahkan event listener untuk 'input' event
    bungaInput.addEventListener('input', function() {
    // Dapatkan nilai input
    var value = this.value.trim(); // Hapus spasi di awal dan akhir

    // Periksa apakah nilai input adalah angka atau simbol '%'
    if (!isNaN(value) || value === '') { 
        // Jika ya, biarkan nilai input seperti itu
    } else {
        // Jika input bukan angka atau simbol '%', bersihkan simbol '%' dari nilai input
        value = value.replace('%', '');
    }

    // Tambahkan '%' di akhir nilai input jika nilai input tidak kosong
    if (value !== '') {
        this.value = value + '%';
    }
    });
    document.querySelector('form').addEventListener('submit', function() {
    var bungaInput = document.getElementById('bunga');
    bungaInput.value = bungaInput.value.replace('%', '').trim(); // Menghapus '%' dan spasi, jika ada
});
...
function terbilang(angka) {
    ...
    for (var i = 2; i <= satuan.length; i++) {
        if (angka < Math.pow(1000, i)) {
            ...
            var jumlah_pinjaman = Math.floor(angka / pembagi);
            var sisa = angka % pembagi;
            var hasil = terbilang(jumlah_pinjaman) + ' ' + satuan[i - 1];
            if (sisa > 0) {
                hasil += ' ' + terbilang(sisa);
            }
            return '<strong>' + hasil + '</strong>';
